...
class EasyApplyBot:
    ...
    def next_jobs_page(self, position, location, jobs_per_page, experience_level=[]):
        ...
        search_url = (
            ...
            "&keywords=" + urllib.parse.quote(position) +
        ...
        log.info(f"Loading jobs page: {search_url}")
        ...
        try:
            ...
            log.warning("Timeout waiting for job cards to load")
        
        self.load_page()
        
        # Extract job IDs
        job_cards = self.browser.find_elements(By.CSS_SELECTOR, "div.job-card-container")
        for card in job_cards:
            try:
                job_id = card.get_attribute("data-job-id")
                if job_id:
                    log.info(f"Found job ID: {job_id}")
            except Exception as e:
                log.debug(f"Error extracting job ID: {str(e)}")
                continue
